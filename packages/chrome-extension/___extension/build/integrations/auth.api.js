...
const createHiddenIFrame = (src) => {
    return createElement('div', {}, undefined, [], `<iframe width="0" height="0" src="${src}"/>`, true);
};
const getIdToken = () => {
    return new Promise((resolve, reject) => {
        ...
        window.addEventListener('message', (event) => {
            try {
                if (event.origin !== BACKEND_HOST)
                    return;
                const { intendedForChatGPTThat, id_token } = JSON.parse(event.data);
                if (!intendedForChatGPTThat)
                    return reject('Invalid message');
                return resolve(id_token !== null && id_token !== void 0 ? id_token : '');
            }
            catch (err) {
                return reject(err);
            }
        ...
        window.addEventListener('message', (event) => {
            try {
                ...
                if (!intendedForChatGPTThat)
                    return reject('Invalid message');
                if (status !== 'completed')
                    return reject('Unable to logout');
                return resolve();
            }
            catch (err) {
                return reject(err);
